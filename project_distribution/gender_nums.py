...
def gen_gender_output(df):   
    ...
    # Step 1: Group the data based on the "Gender" column and calculate count and percentage
    df_clean = df.fillna(0)
    df_clean["Gender"] = df_clean["Gender"].apply(lambda x:'MALE' if (x=='M') else 'FEMALE' if (x=='F') \
            else "OTHER" if (x == "O") else "UNKNOWN")

    grouped_data = df_clean.groupby("Gender").size()
    ...
    arrays = [np.array(["Gender" for x in grouped_data.index]),
